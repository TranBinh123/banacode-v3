...
export function WarmupPage({ teams }: Props) {
  ...
  const [usedTeamIds, setUsedTeamIds] = useState<TeamId[]>([]);
  ...
  const allTeamsPlayed = usedTeamIds.length >= teams.length;
  ...
  const selectTeam = useCallback(
    (teamId: TeamId) => {
      if (active || usedTeamIds.includes(teamId)) return;
  ...
  const startWithSet = useCallback(
    (setId: string) => {
      if (active || !selectedTeamId) return;

      if (usedSetIds.includes(setId)) {
        window.alert("Bộ câu hỏi này đã được sử dụng.");
        return;
      }
