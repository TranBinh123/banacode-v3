...
export function FinishPage() {
  ...
  const handleMarkWrong = () => {
    if (!finish.currentTeamId || !currentQuestion) return;
    // Nếu có sao, đội chính bị trừ điểm
  ...
  const handleStealCorrect = () => {
    ...
    // Đội chính bị -điểm (nếu có sao thì đã trừ ở handleMarkWrong, nhưng vẫn trừ thêm vì luật)
    // Theo luật: nếu đội cướp đúng, đội chính bị trừ điểm (dù có sao hay không)
        ...
        <div className="finish-page-inner">
              ...
              <div className="finish-assignment-package-list">
                {finish.packages
                  ...
                  .map((pkg) => (
                    ...
                    >
                      <div className="finish-package-select-number">{pkg.label.replace("GÓI ", "")}</div>
          ...
          <footer className="finish-public-footer">
            <div className="finish-scoreboard">
              {teams.slice().sort((a, b) => b.totalScore - a.totalScore).map((team) => (
      ...
      <section className="finish-page">
        <div className="finish-finale">
          ...
          <div className="finish-final-scoreboard">
            {teams.slice().sort((a, b) => b.totalScore - a.totalScore).map((team, index) => (
      ...
      <div className="finish-page-inner">
        ...
        <header className="finish-game-header">
          ...
          <div className="finish-game-meta">
            ...
            <div>
              <span>ĐỘ KHÓ</span>
              <strong style={{ color: difficultyColor[currentQuestion.difficulty] }}>
        ...
        <main className="finish-question-area">
          {/* Intro */}
          {isIntro && (
            <div className="finish-intro-panel">
              <div className="finish-intro-badge">
                <span>CÂU HỎI {finish.currentQuestionIndex + 1}</span>
                <span className="finish-difficulty-badge" style={{ backgroundColor: difficultyColor[currentQuestion.difficulty] }}>
                  {difficultyLabel[currentQuestion.difficulty]}
                </span>
          ...
          {isPlaying && (
            <>
              <div className="finish-question-topline">
                ...
                <span style={{ fontFamily: 'monospace', fontSize: '24px', fontWeight: 'bold' }}>
          ...
          {isResolved && (
            <div className="finish-next-panel">
              <div className="finish-result-message">
                <span>✅ ĐÁP ÁN: <strong>{currentQuestion.answer || "Chưa nhập đáp án"}</strong></span>
              </div>
              <button
